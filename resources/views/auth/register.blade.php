<x-guest-layout>
    <div class="text-center mb-8">
        <h1 class="text-2xl font-[Playfair_Display] font-bold text-stone-900">Join Rose Venus</h1>
        <p class="mt-1.5 text-sm text-stone-400">Create your account and start exploring</p>
    </div>
    ...
    <form method="POST" action="{{ route('register') }}" x-data="{ showPassword: false, showConfirmPassword: false }">
        ...
        <div class="mt-5">
            ...
            <div class="relative">
                <input id="password_confirmation" :type="showConfirmPassword ? 'text' : 'password'" name="password_confirmation" required autocomplete="new-password"
                    class="w-full px-4 py-3 pr-11 border border-stone-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-rose-500/20 focus:border-rose-500 transition-all duration-200 placeholder-stone-300"
                    placeholder="&#8226;&#8226;&#8226;&#8226;&#8226;&#8226;&#8226;&#8226;">
                <div @click="showConfirmPassword = !showConfirmPassword" class="absolute inset-y-0 right-0 flex items-center justify-center w-10 cursor-pointer text-stone-400 hover:text-stone-600 transition-colors select-none">
        ...
        <p class="mt-6 text-center text-sm text-stone-400">
            ...
            <a href="{{ route('login') }}" class="text-rose-600 hover:text-rose-700 font-medium transition-colors">Sign in</a>
        </p>
    </form>
</x-guest-layout>